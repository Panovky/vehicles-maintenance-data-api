...
@router.delete(
    '/services/{service_id}/workers/{worker_id}',
    status_code=status.HTTP_204_NO_CONTENT,
    responses={
        204: {'description': 'The worker successfully detached from the service'},
        401: {'description': 'Access token are invalid'},
        403: {'description': 'Access for current user denied'},
        404: {'description': 'Service or worker not found'}
    },
    summary='Detach the worker from the service'
)
async def detach_worker(
    ...
    await service_workers_service.detach_worker(service_id, worker_id)
    return Response(status_code=status.HTTP_204_NO_CONTENT)
